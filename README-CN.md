...
基于 XferCore 引擎的现代化下载管理器，针对 Windows、macOS 和 Linux 深度优化。支持 HTTP、FTP、BitTorrent、磁力链接和 ED2K（电驴）链接，提供专业级功能：ED2K 多方式找源、UPnP/NAT-PMP 端口映射、Tracker 自动更新、任务优先级管理、批量操作和高级下载预设。
...
- 程序默认内置并使用 XferCore 下载引擎（基于 aria2 深度定制，新增原生 ED2K 协议支持），应用会根据引擎的连接上限自动调整"单服务器最大连接数"，以保证稳定与兼容。
- XferCore 的"单服务器最大连接数"上限为 128（默认 32），单个任务最多支持 128 个分片并发；引擎启用 128MB 磁盘缓存与 HTTP keep-alive / pipelining，优化多连接下载性能。
...
- **高速下载**：针对最大下载性能进行了优化，引擎默认启用 128MB 磁盘缓存与 HTTP keep-alive / pipelining
...
- **HTTP/HTTPS**：直接从 Web 服务器下载，支持 keep-alive 与 pipelining 加速
- **FTP/SFTP**：从 FTP 服务器传输文件
- **BitTorrent**：完整支持种子文件，可选择性下载；内置 Tracker 列表与多 DHT 引导节点，开箱即用
- **磁力链接**：无需 .torrent 文件即可直接下载
- **ED2K（电驴）**：原生支持 `ed2k://` 链接，通过 ED2K 服务器、Peer 源交换和 KAD 网络查找下载源

### ED2K 下载

- **多方式找源**：支持 ED2K 服务器找源（传统方式）、Peer 源交换（不依赖服务器）和 KAD 网络（Kademlia DHT，实验性）三种方式并行查找来源
- **服务器订阅**：支持订阅 server.met / 文本格式的服务器源，可手动同步或按设置的间隔/时间自动同步，内置 9 个常用 ED2K 服务器
- **可调参数**：可配置监听端口（默认 4662）、最大连接数、连接超时、每文件最大来源数
- **ED2K 任务详情**：任务详情中展示 ED2K 信息（文件哈希）与实时源列表，可查看每个源的状态、队列位置和可用分片，KAD 网络状态一目了然

### BT/磁力增强

- **传输协议**：支持 uTP 传输协议开关，提升 BT 连接的稳定性与速度
- **网络发现**：UPnP 映射失败时可通过 NAT-PMP 自动映射端口
- **Tracker 预置**：引擎预置 30+ 常用 Tracker 地址，新装即有可用 Tracker
- **DHT 多引导节点**：使用多个内置 bootstrap 节点引导 DHT 网络，连接更可靠
- **请求超时优化**：BT 请求超时设置为 30 秒，更快释放被卡住的 Peer 请求槽

### 视频下载

- **在线视频下载（浏览器扩展）**：通过浏览器扩展嗅探视频资源，一键发送到应用创建下载任务（会携带必要请求头以提高可用性）
- **下载拦截**：浏览器扩展内置下载拦截器，网页中标记了下载（`<a download>` 或常见下载扩展名）的链接点击后可直接转交 LinkCore 接管，支持排除指定域名/扩展名、Alt+点击放行
- **视频嗅探增强**：识别 m4s / mpd 等新格式与 HLS/DASH 清单，区分抖音音/视频并标注 H.264
- **统一任务管理**：视频资源以普通下载任务进入任务列表，支持与其他任务一致的暂停/恢复/删除等管理体验
- **合并进度展示**：需要合并的音视频（如 DASH 视频）下载完成后进入"合并中"状态并显示合并进度，支持外部扩展一次性发送多个 DASH 视频时正确触发合并
...
- **任务调度**：设置下载时间和优先级
- **批量下载**：导入和导出下载列表
- **更新通道**：可选择正式版（stable）/ 最新预发布（beta）/ 全部（all）更新渠道，更新包下载完成后自动安装
...
- **ED2K 服务器自动同步**：定期从订阅源获取最新的 ED2K 服务器列表
