...
基于自研 XferRust 下载引擎（Rust 原生实现）的现代化下载管理器，针对 Windows、macOS、Linux 和 Android 深度优化。下载走 **HTTP(S)** 与 **BitTorrent**（含磁力链接）两条路径，提供 uTP 传输、自动端口映射、Tracker 自动同步、任务优先级管理、批量操作和高级下载预设。网页里的 `ed2k://`、`thunder://` 链接也会被识别并交给应用接管。
...
- **Linux** (x64, arm64)
- **Android** (arm64)：与桌面版共用同一下载引擎，界面针对移动端重新设计，并提供独立的更新渠道选择（稳定版 / 测试版 / 全部版本）
