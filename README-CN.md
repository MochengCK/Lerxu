...
基于自研 XferRust 下载引擎（Rust 原生实现）的现代化下载管理器，针对 Windows、macOS、Linux 和 Android 深度优化。下载走 **HTTP(S)**、**BitTorrent**（含磁力链接）与 **HLS（M3U8）** 三类任务，提供 uTP 传输、自动端口映射、Tracker 自动同步、任务优先级管理、批量操作和高级下载预设。网页里的 `ed2k://`、`thunder://` 链接也会被识别并交给应用接管；桌面端可配合浏览器扩展识别网页视频，Android 端则内置了带资源嗅探与原生播放器的浏览器。
...
- Lerxu 以**独立进程**方式运行 XferRust 引擎，通过本机 JSON-RPC（`127.0.0.1:16800`）驱动；应用会按当前引擎自动匹配最佳的“单服务器最大连接数”策略。
- 单个任务最多支持 **128 个分片并发**（单服务器默认 32 连接），引擎内置 **128M 磁盘缓存**与连接复用，多任务并行下载时更流畅。
- HLS（M3U8）任务由引擎直接拉取清单与分片并拼接为单个文件：清晰度、分片并发（上限 64）、落盘方式与分片重试均可在偏好设置中调整，支持断点续传。
- 默认最多同时管理 **10 个下载任务**，可在偏好设置中调整。
- 单个种子默认最多 **128 个节点连接**（引擎上限 200）。
- 引擎崩溃后自动重启（最多 5 次，带退避），任务与会话状态自动恢复；重启后未完成任务保持**暂停**，可手动恢复。
...
- **HLS（M3U8）**：清单交给引擎自行解析并按播放列表下载分片，自动拼接为单个文件；支持多档码率选择、AES-128 加密分片、分片并发与重试配置，以及断点续传
- **BitTorrent**：完整支持种子文件，可选择性下载；uTP/TCP 双传输、DHT、LPD、PeX 与加密开箱即用
- **磁力链接**：无需 .torrent 文件即可直接下载，元数据就绪后可自由勾选文件
- **请求头**：支持为单个任务设置 `Referer`、`User-Agent` 以及任意自定义请求头，需要它们的地址也能正常下载
- **链接接管**：识别网页中的 `ed2k://` 与 `thunder://` 链接并交给应用处理（`thunder://` 解码后按普通 HTTP 下载，默认关闭，需在偏好设置中开启）
- **不支持**：FTP/SFTP 与 Metalink —— 引擎只实现了 HTTP(S) 与 BitTorrent 两条传输路径。`ed2k://` 仅保留链接识别与浏览器接管，原生 ED2K 下载能力尚未实现。
...
- **节点管理**：任务详情实时展示节点列表（客户端识别、分片进度），可手动封禁问题节点，并支持自定义 IP 封禁名单
- **文件选择**：磁力与种子任务在元数据就绪后可自由勾选要下载的文件
- **做种控制**：可按分享率与做种时长设置目标，完成后自动暂停或保持做种

### 视频下载

- **在线视频下载（浏览器扩展）**：通过浏览器扩展识别网页视频，一键发送到应用创建下载任务；支持 Chrome、Edge、Opera 等 Chromium 内核浏览器与 Firefox（已上架 Firefox 官方扩展商店）
- **下载接管**：网页中的下载链接（如设置了下载属性或以常见文件格式结尾的链接）点击后可直接转交 Lerxu 接管，支持排除指定网站或文件类型、快捷键临时放行
- **视频识别**：支持多种视频格式，含 DASH（自动区分并配对音视频流，下载完成后合并）与 HLS（M3U8 清单整条交给引擎，不展开分片）
- **统一任务管理**：视频资源以普通下载任务进入任务列表，支持与其他任务一致的暂停/恢复/删除等管理体验
- **合并进度展示**：需要合并的音视频下载完成后进入“合并中”状态并显示合并进度，一次发送多个分段视频也能正确合并；FFmpeg 状态可在偏好设置中查看

### 安卓内置浏览器

- **内置浏览器**：Android 端自带浏览器，支持多标签页、历史记录与下拉刷新，可在下载器与浏览器之间随时切换
- **资源嗅探**：浏览网页时自动收集页面上的媒体地址，集中显示在资源面板中，一键发送到下载引擎
- **原生播放器**：网页视频可用应用内置播放器全屏播放，支持播放/暂停、双击快进快退、倍速与画面比例、循环播放、屏幕常亮，以及锁定屏幕防止误触；播放器内另带下载入口，可把当前视频连同文件名与预估大小一并交给引擎
- **无痕窗口**：以独立身份打开无痕窗口，进入前需通过系统生物识别或设备密码验证
- **广告拦截**：默认拦截广告请求并隐藏广告位，可在设置中关闭
- **搜索引擎与默认浏览器**：按当前网络环境自动选择合适的搜索引擎；可设为系统默认浏览器，接管系统中的 http / https 链接
- **默认入口**：可设置应用启动后进入下载器还是浏览器
...
- **任务详情抽屉**：概览、活动、Tracker、节点、文件与 BT 分片图一站式查看
- **独立进度窗口**：任务进度可在独立窗口中查看，支持“仅首个任务”或“所有任务”自动打开
- **完成弹窗**：下载完成时弹出独立完成窗口（可在偏好设置中开关）
- **剪贴板自动粘贴**：复制下载链接后自动填入新建任务
- **系统托盘集成**：快速访问和状态监控
- **下载通知**：下载完成时实时提醒，可配置点击动作
- **任务计划**：可定时执行下载任务的开始/暂停，并设置全部完成后的动作（关机、休眠、退出应用）
- **速度控制**：设置上传和下载速度限制
- **文件管理**：按类别和位置组织下载文件，支持收藏目录与历史目录
- **拖拽下载**：把链接或种子文件拖到窗口即可创建任务
- **多语言**：简体中文、繁体中文、英文
...
- **批量操作**：多选任务后统一暂停、恢复或移除
- **下载安全扫描**：下载完成后可调用系统杀毒工具或自定义扫描工具扫描文件
- **代理支持**：系统代理或自定义代理，可按下载场景分别生效（下载、应用更新、Tracker 更新）
- **更新通道**：可选择正式版、预览版或全部版本更新渠道，更新包下载完成后自动安装
- **引擎信息**：展示当前引擎、版本、架构，以及由引擎自行上报的能力清单
- **本机 RPC 服务**：可选开启 JSON-RPC 服务，让外部工具驱动同一个引擎
...
- **Linux** (x64, arm64)
- **Android** (arm64)：与桌面版共用同一下载引擎，界面针对移动端重新设计；除下载器外还内置浏览器（多标签、资源嗅探、原生播放器、无痕窗口、广告拦截），可设为系统默认浏览器，并提供独立的更新渠道选择（稳定版 / 测试版 / 全部版本）
...
2. 下载最新版本的 `Lerxu-Setup-<版本号>.exe` 安装程序
3. 运行安装程序并按照屏幕提示完成安装
...
| `npm run build:applesilicon` | 构建 macOS arm64 目标 |
| `npm run build:github` | 只构建前端产物（CI 用，不出安装包） |
| `npm run release` | 构建并发布（CI 发版用） |
| `npm run preview` | 本地预览已构建的前端产物 |
| `npm run build:extension` | 打包浏览器扩展（Chromium + Firefox） |
| `npm run test:extension` | 运行浏览器扩展测试 |
| `npm run lint:extension` | 用 `web-ext` 校验打包后的 Firefox 扩展 |
| `npm run lint` | 对 `src` 执行 ESLint |
| `npm run lint:fix` | ESLint 自动修复 |
| `npm run lint:workflows` | 校验 GitHub Actions 工作流（YAML 语法 + 表达式语义） |
...
引擎源码（独立仓库）、引擎二进制回填、Android 与浏览器扩展的构建方式、测试与 CI 配置的说明见 [开发文档](docs/DEVELOPMENT.md)。
